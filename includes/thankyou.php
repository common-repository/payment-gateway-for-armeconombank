...
add_action('woocommerce_thankyou', 'woocomerceShowErrorMessageArmeconombank', 4);
 ...
 * @return bool
 */
function woocomerceShowErrorMessageArmeconombank($order_id)
{
    $order = wc_get_order($order_id);
    if ($order->has_status('failed')) {
        $orderFailedMessage = get_post_meta($order_id, 'FailedMessageArmeconombank', true);
